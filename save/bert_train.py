...
def train_or_test(model,data,label,data_index,batch_size,train_type,optimizer,epoch,lossF=nn.NLLLoss(reduction='sum')):#F.nll_loss
    ...
    for i in range(0,len(data),batch_size):
        data_batch,label_batch,_ = bert_utils.get_minibatch(data,label,data_index,i,batch_size,train_type)
    ...
    if train_type=='valid':
        ...
        utils.saveResult(Y,preds,'Bert_Result_100_%d'%(epoch))
        #np.save('Bert_Result_100.npy',test_scores)
...
"KFold代码"
#train_data,label,test_data,ids
train_data = np.array(train_data[:1000])
label = np.array(label[:1000])
...
for epoch in range(1,epoches+1):
    ...
    for i,(train_index,valid_index) in enumerate(skf):
        ...
        train_or_test(model,xtrain,ytrain,train_data_index,batch_size,'train',optimizer,i)
        train_or_test(model,xvalid,yvalid,valid_data_index,batch_size,'valid',optimizer,i)
        test_scores,Y = train_or_test(model,test_data,ids,test_data_index,batch_size,'test',None,i)
    ...
    utils.saveResult(Y,aver,'Bert_Result_100_%d'%(epoch))
'''